...
@section('breadcrumbTitle', 'Edit Job Posting')
        ...
        <div class="card form-card shadow-sm">
            <div class="card-body p-4 p-md-5">
                <form action="{{ route('careers.update', $career->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-lg-8">
                            <h4 class="mb-4 text-dark fw-bold"><i class="fas fa-edit text-primary me-2"></i> Edit Career Listing</h4>
                            
                            <div class="mb-4">
                                <label class="form-label">Job Title <span class="text-danger">*</span></label>
                                <input type="text" name="title" class="form-control" placeholder="e.g. Business Strategy Consultant" value="{{ old('title', $career->title) }}" required>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <input type="text" name="job_type" class="form-control" placeholder="e.g. Full time job" value="{{ old('job_type', $career->job_type) }}">
                                    @error('job_type') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>
                                <div class="col-md-6 mb-4">
                                    <label class="form-label">Category (e.g. On-site)</label>
                                    <input type="text" name="category" class="form-control" placeholder="e.g. On site / Remote" value="{{ old('category', $career->category) }}">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <input type="text" name="salary" class="form-control" placeholder="e.g. $400 - $550" value="{{ old('salary', $career->salary) }}">
                                    @error('salary') <small class="text-danger">{{ $message }}</small> @enderror
                                </div>
                                <div class="col-md-6 mb-4">
                                    <label class="form-label">Salary Unit</label>
                                    <input type="text" name="salary_unit" class="form-control" placeholder="e.g. week / month" value="{{ old('salary_unit', $career->salary_unit) }}">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="location" class="form-control" placeholder="e.g. London, UK" value="{{ old('location', $career->location) }}">
                                @error('location') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>

                            <div class="mb-4">
                                <label class="form-label">Short Description</label>
                                <textarea name="short_description" class="form-control" rows="3" placeholder="Brief overview of the vacancy...">{{ old('short_description', $career->short_description) }}</textarea>
                                @error('short_description') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>

                            <div class="mb-4">
                                <label class="form-label">Job Details (Rich Text)</label>
                                <textarea id="summernote" name="description">{{ old('description', $career->description) }}</textarea>
                            ...
                            <div class="p-3 bg-light rounded-4 mb-4">
                                ...
                                <div class="mb-4 d-flex align-items-center">
                                    <div class="form-check form-switch card-radio-custom">
                                        <input class="form-check-input" type="checkbox" name="is_urgent" id="is_urgent" {{ old('is_urgent', $career->is_urgent) ? 'checked' : '' }}>
                                        <label class="form-check-label fw-bold ms-2" for="is_urgent">Mark as Urgent</label>
                                    </div>
                                </div>

                                <div class="mb-4">
                                    <label class="form-label">Current Icon / Feature Image</label>
                                    ...
                                    <div class="mt-2 text-center border rounded-3 p-2 bg-white" style="min-height: 100px; display: flex; align-items: center; justify-content: center;">
                                        @if($career->feature_image)
                                            <img id="featureImagePreview" src="{{ asset($career->feature_image) }}" alt="Preview" style="max-width: 100%; max-height: 150px; border-radius: 8px;">
